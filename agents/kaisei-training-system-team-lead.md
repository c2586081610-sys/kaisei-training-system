...
- **已有能力**：
  ...
  - 调取用户 Apple Health 数据：HRV / RHR / 睡眠 / 体重 / 活动量（567 天睡眠 / 1352 天活动 / 43 天体重 / 19 天 HRV / 15 天 RHR & SpO2）
...
- **已有能力**：
  ...
  - **调取 Apple Health 恢复指标**：RHR / HRV / 睡眠（567 天）/ 血氧 / 活动量 / 饮水 — 综合判断恢复状态
...
- **已有能力**：
  ...
  - **调取 Apple Health 异常信号**：SpO2 偏低 / RHR 持续偏高 / HRV 异常 — 提示可能伤痛/疾病风险
...
**当前可跑通的端到端流程**：
- 训练：读用户训记历史 + Apple Health 活动/睡眠 + 查动作库 + 写回训练（dry_run 三步）
- 营养：碳循环计算（用 Apple Health 体重） + 读用户训记饮食 + 搜索食物 + 写回饮食 + 饮水追踪
- 恢复：查主动恢复动作 + 训记训练量 + Apple Health RHR/HRV/睡眠综合判断
- 防护：查动作注意段 + 训记训练频率 + Apple Health 异常信号（SpO2/RHR 异常）

**当前跑不通的**（待 Phase 5 蒸馏填充）：
- 综合训练方案（缺周期化/分化方法论）
- 训练过度综合判断的方法论（数据已齐，方法论待蒸馏）
- 损伤康复具体方案（缺康复蒸馏）
